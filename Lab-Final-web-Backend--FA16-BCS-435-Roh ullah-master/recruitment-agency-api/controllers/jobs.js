...
exports.deleteJob = async (req, res) => {
  const body = req.body;
  try {
    const response = await Jobs.findByIdAndDelete(body.id).populate(
      "applications.user",
      "firstName lastName email user_details"
    );
    if (response) {
      await res.json({
        success: true,
  ...
  try {
    ...
    if (response) {
      ...
      const emailData = {
        to: email,
        subject:
          "Applied for Job | E-recruitment System for Ministry of Agriculture Irrigation and Live-stock",
        html: `
          <p>Dear User,</p>
...
exports.scheduleTestInterview = async (req, res) => {
  ...
  try {
    const response = await Jobs.findOneAndUpdate(
      ...
      },
      {
        ...
        arrayFilters: [
          {
            "elem._id": {
              $in: applicationsIds.map((id) => mongoose.Types.ObjectId(id)),
            },
          },
        ],
      }
    ...
    if (response) {
      const emailData = {
        to: emails,
        subject: `${type.toUpperCase()} Scheduled | E-recruitment System for Ministry of Agriculture Irrigation and Live-stock`,
        html: `
          <p>Dear Applicant,</p>
          <p>We are pleased to inform you that you are shortlisted for <b>${type.toUpperCase()}</b> for the Job "<b>${
          response.title
        }</b>"</p>
          <p>Your ${type} has been scheduled on:</p>
          <p><b>${type.toUpperCase()} Date: </b>${moment(date).format(
          "DD/MM/YYYY h:mm a"
        )}</p>
          <p>Regards!</p>
          <br/>
          <p>Minstry of Agriculture !</p>
        `,
      };
      sendEmail(emailData);
      await res.json({
  ...
  try {
    ...
    if (response) {
      ...
      const emailData = {
        to: email,
        subject:
          "Job Application Status | E-recruitment System for Ministry of Agriculture Irrigation and Live-stock",
        html: `
          <p>Dear Applicant,</p>
          <p>We ${emotion} to inform you that you have <b>${statusType}</b> the <b>${type}</b> for the Job "<b>${response.title}</b>"</p>
          <p>Regards!</p>
          <br/>
          <p>RMinsitry of Agriculture !</p>
        `,
      };
      sendEmail(emailData);
      await res.json({
    ...
    if (response) {
      if (status === "4" || status === "6") {
        ...
        const emailData = {
          to: emails,
          subject:
            "Job Application | E-recruitment System for Ministry of Agriculture Irrigation and Live-stock",
          html: `
          <p>Dear Applicant,</p>
          <p>We ${emotion} to inform you that you have been <b>${statusType}</b> for the Job "<b>${response.title}</b>"</p>
          <p>Regards!</p>
          <br/>
          <p>Minsitry of Agriculture !</p>
        `,
        };
        sendEmail(emailData);
      }
